...
from cqs.ds
where dsterm in ('Completed', 'Withdrawn')
group by 1,2,3
...
siteid,
usubjid,
max(svstdtc) as "EOT Date"
from cqs.sv
where visit like '%End of Therapy%' or visit like '%End of Treatment%'
group by 1,2,3
),
...
rsv.visitnum
from cqs.rpt_subject_visit_schedule rsv
where (rsv.studyid ,rsv.siteid,rsv.usubjid,rsv.visitnum)
in(
select studyid,siteid,usubjid,max(visitnum) as visitnum from cqs.rpt_subject_visit_schedule
where visit_start_dtc is not null
and 
expected = 'true'
group by 1,2,3
...
and q.usubjid not in (
select usubjid from cqs.query where querystatus = 'Open'
)
